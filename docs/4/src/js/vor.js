...
class Validator {
    ...
    static rangeOf(target) {return RangeOfValidator.get(target)}
...
class TypeOfValidator extends OfValidator {
    ...
    is(fn) {return #run('is', fn)}
    expect(fn) {return #run('expect', fn)}
    #run(methodName, fn) {
        const a = fn();
...
class ValueOfValidator extends OfValidator {
    constructor(target) {super(target)}
    is(...candidates) {return #run('is', ...candidates)}
    expect(...candidates) {return #run('expect', ...candidates)}
...
class RangeOfValidator extends OfValidator {
    ...
    is(min, max) {return #run('is', min, max)}
    expect(min, max) {return #run('expect', min, max)}

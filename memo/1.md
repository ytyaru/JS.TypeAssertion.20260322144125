...
```javascript
Type.assert(target, fn);    // 指定した型であること（）
Type.validate(target, fn);  // 指定した型であること（）
Type.verify(target, fn);    // 指定した型であること（）

Type.is(target, fn);        // 指定した型であること（真偽値を返す）
Type.throw(target, fn);     // 指定した型であること（例外発生）
Type.to(target, fn);        // 指定した型に変換すること
```
```javascript
// 複数の型のいずれかである場合
Type.is(target, fn1, fn2, ...)
```
```javascript
// 特定の型である場合
Type.is(target, String);
// undefined, null, true, false, 0, NaN, Infinity, -Infinity, ''
// Boolean, Number, BigInt, String, Symbol
// 他クラス名
```
```javascript
// 特定の型のいずれかである場合
Type.is(target, null, String);
// undefined, null, true, false, 0, NaN, Infinity, -Infinity, ''
// Boolean, Number, BigInt, String, Symbol
// 他クラス名
```
